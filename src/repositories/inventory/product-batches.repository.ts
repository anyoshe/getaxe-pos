...
export class ProductBatchRepository
    ...
    async existsByBatchNumber(
        ...
        const batch =
            await this.database.query.productBatches.findFirst({
                where: and(
                    eq(productBatches.businessId, businessId),
        ...
        const result =
            await this.database
                .select({
                    quantity: sql<number>`
            COALESCE(
              SUM(${productBatches.quantityRemaining}),
              0
            )
          `,
                })
    ...
    async decreaseQuantity(
        ...
        const [batch] =
            await this.database
                ...
                .set({
                    quantityRemaining:
                        sql`${productBatches.quantityRemaining} - ${quantity}`,
                })
                .where(
